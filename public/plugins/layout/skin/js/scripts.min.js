...
var updateChart = function () {
    ...
    //根据表名获得到表内部的ID32
var getTableAllId32 = function (tableId, callback) {
    $.ajax({
        url: '/api/source/' + tableId,
        type: 'GET',
        success: function (result) {
            $("#TableId32").empty();
            for (index in result) {
                var option = "<option value='" + result[index].key + "'>" + result[index].value + "</option>";
                $("#TableId32").append(option);
            }
            callback;
        }
    })
...
$(document).ready(function () {
    ...
    $("#Table").change(function () {
        $("TableId32").empty();
        getTableAllId32($(this).children('option:selected').val());
    });
    ...
    ***/
    ...
    $('body.edit .demo').on("click", "[data-target=#editorModal]", function (e) {
        ...
        currentContent = $(this).parent().parent().find(".labelContent");
        //currentContent = $('#labels');
        var tabid = currentContent.data("source");
        var id32 = currentContent.data("id");
        if (tabid) {
            getTableAllId32(tabid, function () {
                $("#Table").val(tabid);
                $("#TableId32").val(id32);
            });
        } else {
            getTableAllId32("digit", function () {
                $("#Table").val("digit");
                $("#TableId32").val("");
            });
        }
    ...
    $("#savecontent").click(function (e) {
        e.preventDefault();
        //currentContent = $('#labels');
        var tabid = $("#Table").children('option:selected').val();
        var Id32 = $("#TableId32").children('option:selected').val();
        currentContent.attr("data-source", tabid);
        currentContent.attr("data-id", Id32);
        //调用显示函数，将数据显示在微标上。
        //updateChart();
    });
    ...
    $('body.edit .demo').on("click", "[data-target=#canvasModal]", function (e) {
        ...
        if (canvasId) {
            ...
            updateCanvasElement("bar", function () {
                $("#canvasType").val("bar");
                $("#canvasElement").val("");
            });
        }
